...
1. 대면 경쟁의 **차량 클래스**는 다음 제약 조건을 충족하는 차량만 허용합니다:
    ...
    6. _LiDAR_:
        [**Hokuyo UST-30LX**](https://www.hokuyo-aut.jp/search/single.php?serial=167), i동급 또는 그보다 낮은 사양의 모든 라이다를 허용합니다. 관찰된 주요 특성은 다음과 같습니다. 감지 범위(30m), 스캔 주파수(40Hz) 및 각도 분해능(0.25°)입니다.
...
1. 바닥의 표면은 평평하고 러그가 깔려 있습니다.
2. 방은 가벽을 사용하여 강당을 분할한 공간입니다. 벽은 빛이 통과하지 않는 소재로 이루어져 있습니다. 벽에 창문이 없기 때문에, 외부의 빛이 들어오지 않습니다.
3. 트랙 경계는 폴리에스터와 철사로 만들어진 직경 50cm의 공기 파이프로 구성하고 있으며, 케이블 타이와 마스킹 테이프로 고정되어 있습니다. LiDAR 빔이 통과할 수 있는 파이프 사이에 간격이 있을 수 있을 수 있습니다.
4. 트랙은 약 25×10 m의 면적에 들어 갈 것 입니다.
5. 각 팀별 **매핑** 시간은 대회 규모에 따라 대략 3분에서 5분 정도 배정할 예정입니다.

## 2.3 차량 점검

1. 차량 점검의 목적은 자율주행 자동차의 하드웨어가 경주 요구 사항을 충족하는지, 자동차가 환경, 상대 및 사람에게 위험하지 않은지 확인하는 것입니다.

2. 차량 점검은 경주 첫 날에 합니다.
...
2. 예선은 2번의 기회가 주어집니다. 각 예선은 5분 동안 가능한 한 바퀴를 빨리, 가능한 많은 바퀴를 움직이는 것입니다. 이 때 자동차가 충돌 혹은 정지하는 경우에도 타이머는 멈추지 않습니다.

3. 대회의 규모에 따라 기회가 한번 주어질 수 있습니다.
...
7. 동점인 경우에는 가장 빠른 기록 순위가 우선시 됩니다.
...
5. 각 라운드는 3판 2선으로 진행이 됩니다.

6. 각 라운드의 첫번째 게임의 시작위치은 예선 순위가 높은 팀이 결정하고 그 이후는 직전 게임의 진 팀에서 고릅니다.

7. 하나의 일대일 경기는 두개의 팀이 서로 경쟁하는 것으로 진행됩니다. 한 경주는 약 5분 정도의 타임슬롯을 가지고 있습니다. 만약 한 팀이 5분 안에 나타나지 않고, 경기가 진행된다면, 반대편 팀이 승리하게 됩니다. 경기 진행 중 어느 시점에 차량이 더 이상 주행할 수 없는 경우(예 : 하드웨어 문제, 소프트웨어 작동 안 함 등), 5분 이내에 차량을 다시 출발시킬 수 없다면 반대편 팀이 승리하게 됩니다. 
별도의 시간 연장은 주어지지 않고 5분이 지나면 다음 차례의 팀이 경기를 진행하게 됩니다.


8. 경쟁하는 차들은 각각의 출발선에서 출발합니다. 각 차량의 출발선은 트랙 안에서 서로의 반대편에 위치합니다.

9. 추월은 오른쪽 또는 왼쪽에서 수행할 수 있습니다.

10. 타임 트라이얼 경기와 달리, 아래 설명과 같이 충돌 후를 제외하고는 레이스 중에 차량 재구성이 불가능합니다.

11. 최종적으로, 주최 측은 일대일 경기에서 차량 충돌 시 책임을 물을 권리가 있습니다.
...
4. 상대를 충돌하면 다음 단계가 적용됩니다:
    ...
    1. 심판은 어떤 차에 잘못이 있는지 판단하고 경고를 줍니다.
    2. 두 대의 차는 심판이 정한 장소에서 놓입니다.
    3. 심판이 경기를 재개합니다.
    4. 한 게임에서 두번의 경고를 받는다면, 그 게임은 지고 다음 게임을 진행합니다.
